...
class EIAAPI:
    ...
    def fetch_route_details(self, route_id, sub_route_id=None):
        if sub_route_id:
            url = f"{self.base_url}{route_id}/{sub_route_id}/"  # Use sub-route endpoint correctly
        else:
        ...
        try:
            ...
            response.raise_for_status()
            return response.json()["response"]  # Access the response part correctly
        except requests.RequestException as e:
    ...
    def fetch_facet_options(self, route_id, facet_id):
        ...
        try:
            ...
            # Extract facet values if available
    ...
    def fetch_data(self, route_id, frequency, facets, data_fields, start_date=None, end_date=None, max_rows=None):
        ...
        Args:
            ...
            facets (dict): A dictionary of facet IDs to their selected values.
        ...
        if start_date:
            try:
                ...
                params["start"] = adjusted_start_dt.strftime('%Y-%m-%d')
            except ValueError:
